...
//
// This caused a state-leak bug: when an Admin logged out and a Maintenance user
// logged in on the same browser tab (without a full page reload), the Admin's
// synced endpoint blocked the new user's subscription from being registered
// with the correct backend user_id.
//
