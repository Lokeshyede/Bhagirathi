...
export const Login: React.FC = () => {
  ...
  const roleMismatchError =
    searchParams.get("error") === "role_mismatch"
      ? `You are currently signed in as ${
          ROLE_LABELS[searchParams.get("role") ?? ""] ?? searchParams.get("role")
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (tokenData) => {
      try {
        ...
        // No separate /auth/me call needed — eliminates the race condition
        // where /me was called with the wrong portal token key.
        const user = tokenData.user;
        if (!user) {
          // Fallback: fetch /me if the server (older version) does not include user
        ...
        <div className="mt-5 space-y-3 font-semibold text-xs leading-normal">
          {roleMismatchError && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex items-start gap-2.5 p-3.5 bg-amber-50 dark:bg-amber-955/10 text-amber-700 dark:text-amber-400 rounded-2xl border border-amber-250/50 dark:border-amber-900/25"
            >
          ...
          {serverError && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex items-start gap-2.5 p-3.5 bg-red-50 dark:bg-red-955/10 text-[#E53935] dark:text-red-405 rounded-2xl border border-red-200/50 dark:border-red-900/25"
            >
              <ShieldAlert className="h-4.5 w-4.5 shrink-0 mt-0.5" />
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4 text-xs font-semibold text-[#111827] dark:text-zinc-200">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("email")}
              className={`h-[50px] w-full px-4 rounded-xl border bg-white dark:bg-zinc-950 text-stone-850 dark:text-white placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-[#E53935]/15 focus:border-[#E53935] transition duration-150 ${
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative flex items-center">
              <input
                ...
                {...register("password")}
                className={`h-[50px] w-full pl-4 pr-12 rounded-xl border bg-white dark:bg-zinc-950 text-stone-855 dark:text-white placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-[#E53935]/15 focus:border-[#E53935] transition duration-150 ${
              ...
              <button
                ...
                className="absolute right-4 p-1 text-stone-400 hover:text-stone-605 transition cursor-pointer"
          ...
          <input type="hidden" value={UserRole.ADMIN} {...register("role")} />
          ...
          <div className="flex items-center justify-between pt-1 select-none text-[11px] font-bold uppercase tracking-wider">
            <label className="flex items-center gap-2 cursor-pointer text-[#64748B] dark:text-zinc-400">
              <input
                type="checkbox"
                className="accent-[#E53935] h-4.5 w-4.5 rounded border-[#E2E8F0]"
              />
