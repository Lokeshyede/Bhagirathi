...
export const Login: React.FC = () => {
  ...
  const roleMismatchError = searchParams.get("error") === "role_mismatch"
    ? `You are currently logged in as ${ROLE_LABELS[searchParams.get("role") ?? ""] ?? searchParams.get("role")}. Please sign in with a TENANT account to access the Tenant Portal.`
    : null;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#F8FAFC] dark:bg-zinc-955 transition-colors">
      <motion.div
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4 text-xs font-semibold text-[#111827] dark:text-zinc-200">
          ...
          <input type="hidden" value={UserRole.TENANT} {...register("role")} />
