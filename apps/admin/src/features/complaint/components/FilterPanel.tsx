...
import { ComplaintStatus, ComplaintPriority, ComplaintCategory } from "@bhagirathi/constants";
...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const priorities = [
    ...
    { label: "Critical", value: ComplaintPriority.CRITICAL }
  ];

  const categories = [
    { label: "All Categories", value: "ALL" },
    { label: "Electricity", value: ComplaintCategory.ELECTRICITY },
    { label: "Water", value: ComplaintCategory.WATER },
    { label: "Cleaning", value: ComplaintCategory.CLEANING },
    { label: "Furniture", value: ComplaintCategory.FURNITURE },
    { label: "Internet", value: ComplaintCategory.INTERNET },
    { label: "Bathroom", value: ComplaintCategory.BATHROOM },
    { label: "Room", value: ComplaintCategory.ROOM },
    { label: "Security", value: ComplaintCategory.SECURITY },
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 w-full">
      <div className="flex flex-wrap items-center gap-4">
        ...
        {/* Category Dropdown */}
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-bold text-secondaryText dark:text-gray-400 uppercase tracking-widest">Category</label>
          <select
            value={categoryFilter}
            onChange={(e) => onCategoryChange(e.target.value)}
            className="h-9 px-3 bg-gray-50 dark:bg-gray-955 border border-border dark:border-gray-700 rounded-input text-xs font-semibold text-primaryText dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary cursor-pointer transition-all"
          >
            {categories.map((c) => (
