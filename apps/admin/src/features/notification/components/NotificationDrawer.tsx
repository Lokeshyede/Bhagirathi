...
export const NotificationDrawer: React.FC = () => {
  ...
  const { data: notifications = [], isLoading } = useNotifications({
    limit: 10,
    statusFilter: "UNREAD" // Show unread by default in drawer for quick action
  });
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      if (isOpen && drawerRef.current && !drawerRef.current.contains(event.target as Node)) {
        // Only close if not clicking a bell trigger
    ...
    <div className="fixed inset-0 z-50 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
      ...
      <div className="absolute inset-y-0 right-0 pl-10 max-w-full flex">
        ...
        >
          ...
          {/* Drawer Footer */}
          <div className="p-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950/20">
            <Link
              to="/notifications"
              onClick={closeDrawer}
